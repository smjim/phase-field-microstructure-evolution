      ...
      integer, dimension(:), allocatable :: boundary_gl 
...
!     MPI Initializations
      ...
      do step_i = 1, num_steps 
            ...
            do kk=1,dims(2)
               ...
               do i = 1, Nx 
                  ...
                  boundary_gl(ll) = 0  ! Initialize to zero
                  ...
                  if (phi_above_threshold.gt.1) boundary_gl(ll) = 1 ! this is a boundary
               end do
               end do
               end do
            ...
            call analyze_data(Nx, Ny, Nz, phi_3_gl, threshold, data_filename, output_dir, step_string)
      ...
      subroutine analyze_data(Nx, Ny, Nz, phi_3_gl, threshold_percent, output_file, output_dir, step_string)
            implicit none

            integer, intent(in) :: Nx, Ny, Nz
            real *8, dimension(:), intent(in) :: phi_3_gl
            real *8, intent(in) :: threshold_percent 
            real *8 :: max_gradient_value, boundary_val 
            ...
            ! Determine max gradient value
            max_gradient_value = 0
            do k = 2, Nz-1
            do j = 2, Ny-1
            do i = 2, Nx-1
              if (sqrt(grad_phi_3_x(i,j,k)**2 + grad_phi_3_y(i,j,k)**2 + grad_phi_3_z(i,j,k)**2) > max_gradient_value) then
                max_gradient_value = sqrt(grad_phi_3_x(i,j,k)**2 + grad_phi_3_y(i,j,k)**2 + grad_phi_3_z(i,j,k)**2)
              end if
            end do
            end do
            end do

            boundary_val = max_gradient_value * threshold_percent
            ...
            if (boundary_voxels .ge. 1) then ! only make an output file if there are 1 or more boundary voxels
              ...
              print *, boundary_voxels, " boundary voxels detected for ", trim(adjustl(step_string))
            end if
