      ...
      integer j1, j2, nrun, rad_ppt, radius, phi_num, ppt_int, var_count
...
!        mob_c(i,j,k) = mob_c(i,j,k) + fi*D_bulk(ivar)
!     end do

!     if(phi_num.gt.1) mob_c(i,j,k) = mob_c(i,j,k)*phi_num*D_gb
...
!!      k_sq = kf(1,i,j,k)*kf_sq(1,i,j,k) + kf(2,i,j,k)*kf_sq(2,i,j,k) + &
!              kf(3,i,j,k)*kf_sq(3,i,j,k)
!!      dft_dummy(i,j,k) = kf_sum*dft_df_dc(i,j,k) - grad_coeff_c*dft_con(i,j,k)*(kf_sum*k_sq)
!       dft_dummy(i,j,k) = kf_sum*dft_df_dc(i,j,k) - grad_coeff_c*dft_con(i,j,k)*k_sq

        kf_sum = kf(1,i,j,k) + kf(2,i,j,k) + kf(3,i,j,k)
        k_sq = kf_sq(1,i,j,k)+kf_sq(2,i,j,k)+kf_sq(3,i,j,k)
        k_cube = kf(1,i,j,k)*kf_sq(1,i,j,k) + kf(2,i,j,k)*kf_sq(2,i,j,k) &
        + kf(3,i,j,k)*kf_sq(3,i,j,k)
        dft_dummy(i,j,k) = kf_sum*dft_df_dc(i,j,k) - grad_coeff_c* &
                           dft_con(i,j,k)*k_cube
