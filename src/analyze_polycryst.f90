      ...
      integer i, j, k, kk, var, ivar, step, var_max, ii, jj, ll
      integer ix, jy, kz
      integer ierr
      ! --
      ...
      ! --
      ! Calculation of max ppt length
      integer, dimension(:,:), allocatable :: ppt_loc
      integer :: num_ppt_loc
      character(len=256) :: ppt_outfile
...
! There will be approximately 16 data_save.xxxx files
      ...
      do step_i = my_start_step, my_end_step 
            ...
            write(*,'(2A)') "Analyzing step: ", step_string 
            ...
            do kk = 1, dims(2)
              ...
              !$omp& reduction(+:boundary_voxels, +:phi_var_sum, +:precipitate_boundary_voxels, +:num_ppt_loc)
              ...
              do i = 1, Nx 
                ...
                phi_sq_gl(ll) = phi_sq
                if (phi_sq <= threshold) then 
                  num_var_gl(ll) = var_max
                else
                  num_var_gl(ll) = 0 
                end if

                ! Increment num_ppt_loc
                !if (phi(var,i,j,k) >= threshold .and. phi_sq_gl(ll) <= threshold) then
                if (phi(var,i,j,k) >= threshold) then
                  num_ppt_loc = num_ppt_loc + 1
                end if
            ...
            allocate(ppt_loc(num_ppt_loc, 3))
            ppt_loc = 0
            jj = 0 
            ...
            do kk = 1, dims(2)
              do j = 1, dims(1)
                ...
                if (j == 1) allocate( phi(var, Nx, Ny, ist(3):ien(3)) )
                if (j == 1) allocate( con(Nx, Ny, ist(3):ien(3)) )
                !write(*,'(A,9I8)') "myid, kk, j, ist, ien: ", myid, kk, j, ist, ien
                read(12) phi(1:var,1:Nx,ist(2):ien(2),ist(3):ien(3)),  &
                         con(1:Nx,ist(2):ien(2),ist(3):ien(3))
                close(12)
              ...
              do ii = 1, N
                ...
                do jy = 1, Ny
                  ll = (kz-1) * Nxy + (jy-1) * Nx + ix
                  !if (phi(var,ix,jy,kz) >= threshold) &
                  !if (phi_sq_gl(ll) <= threshold .and. phi(var,ix,jy,kz) >= threshold) &
                  if (num_var_gl(ll) == var) &
                ...
                !print *, myid, " ix, kz, intersections ", ix, kz, intersections
              end do
              !print *, myid, "analyzing yz plane"
              ...
              do ii = 1, N
                ...
                !print *, myid, " jy, kz, intersections ", jy, kz, intersections
              end do
              !print *, myid, "analyzing xy plane"
              ...
              do ii = 1, N*dims(2)
                ...
                !print *, myid, " ix, jy, intersections ", ix, jy, intersections
              end do

              ! Determine ppt boundary locations
              do i = 1, Nx
              do j = 1, Ny
              do k = ist(3), ien(3) 
                ll = (k-1) * Nxy + (j-1) * Nx + i
                !print *, "myid, jj, num_ppt_loc: ", myid, jj, num_ppt_loc 
                !if (phi(var,i,j,k) >= threshold .and. phi_sq_gl(ll) <= threshold) then
                if (phi(var,i,j,k) >= threshold) then
                  jj = jj + 1
                  ppt_loc(jj,:) = (/ i, j, k /)
            ...
            phi_var_sum_glob = phi_var_sum ! because phi_var_sum calculation done in series
...
!                MPI_SUM, MPI_COMM_WORLD, ierr)
            intersections_glob = intersections ! because intersections calculation done in series
            total_length = dims(2) * ( (N * (Nx + Ny)) + (dims(2) * N * (ien(3) - ist(3))) )
            N_L = real(intersections_glob, kind=8)/ real(total_length, kind=8)

            ! Calculate average mean thickness prediction
            thickness_est = 2*phi_var_vol_frac/ N_L
            write(93,'(2A,2I8,3F8.4,3I8)') "step, myid, phi_var_sum_glob, thickness_est, phi_var_vol_frac, N_L, total_length, intersections, intersections_glob: ", trim(step_string), myid, phi_var_sum_glob, thickness_est, phi_var_vol_frac, N_L, total_length, intersections, intersections_glob
            write(93,'(2A,2I8,3F8.4,3I8)') "step, myid, phi_var_sum_glob, thickness_est, phi_var_vol_frac, N_L, total_length, intersections, intersections_glob: ", trim(step_string), myid, phi_var_sum_glob, thickness_est, phi_var_vol_frac, N_L, total_length, intersections, intersections_glob
            ...
            write(93,'(A,F8.4)') trim(step_string), thickness_est
            ...
            ! ====================
            ! Grain Boundary Tracking - Determine maximal spreading of each grain
            ! ====================
            ppt_outfile = trim(output_dir) // '/ppt_loc_' // trim(adjustl(step_string)) // '.dat'
            call save_ppt_loc(Nx, Ny, Nz, num_ppt_loc, ppt_loc, ppt_outfile)
            ! ====================
      ...
      subroutine save_ppt_loc(Nx, Ny, Nz, num_ppt_loc, ppt_loc, ppt_outfile)
            implicit none

            integer, intent(in) :: Nx, Ny, Nz, num_ppt_loc
            integer, intent(in) :: ppt_loc(num_ppt_loc, 3)
            character(len=256), intent(in) :: ppt_outfile

            integer :: i, j
            integer :: io_status
            character(len=256) :: format_str
        
            ! Open the output file
            write(*,'(2A)') "Saving ppt locations to ", trim(ppt_outfile)
            open(unit=10, file=trim(ppt_outfile), status='replace', action='write', &
                 form='formatted', iostat=io_status)
            if (io_status /= 0) then
                print *, "Error opening file:", trim(ppt_outfile)
                return
            end if
        
            ! Write num_ppt_loc, Nx, Ny, Nz
            write(10, '(4I10)') num_ppt_loc, Nx, Ny, Nz
        
            ! Write ppt_loc array
            do i = 1, num_ppt_loc
                write(10, '(3I8)') ppt_loc(i, 1), ppt_loc(i, 2), ppt_loc(i, 3)
            end do
        
            ! Close the file
            close(10)

      end subroutine save_ppt_loc
