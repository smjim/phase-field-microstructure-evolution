      ...
      real *8 D_gb, D_mean, phi_sum, fi, D_vol, D_S, D_L
...
!     set initial order parameters
      ...
      phi_count = 0
      do ivar = 1, var
      if(phi(ivar,i,j,k).gt.0.0) phi_count = phi_count + 1
      end do

      if(phi_count.eq.0) write(200+myid,*) i,j,k, phi
