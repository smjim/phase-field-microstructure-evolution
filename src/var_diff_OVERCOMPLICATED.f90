      ...
      integer rank, nprocs, myid, ierr , i_ppt, j_ppt, k_ppt, num_ppt
...
!     set initial order parameters
      ...
      IF(nrun.eq.1) THEN
        ...
        if (start.eq.'polycryst' .or. start.eq.'poly_bonus') then
          call initialize_polycrystal(Nx, Ny, Nz, phi, con, ist, ien, var, bonus_present, con_0_ppt)
...
! it seems to be related to including them in a separate file
...
!     concentration and additional precipitates
      if (bonus_present.and.(step == bonus_start)) then 
        call assign_additional_precipitates(Nx, Ny, Nz, phi, con, ist, ien, bonus_start, start, wetting_param, con_0_ppt, num_ppt, ppt_rad, myid, nprocs, var, ierr)
        print *, "success??"
      end if
...
9     format(3i6, 4e15.7)
      ...
      subroutine assign_additional_precipitates(Nx, Ny, Nz, phi, con, ist, ien, bonus_start, start, wetting_param, con_0_ppt, num_ppt, ppt_rad, myid, nprocs, var, ierr)
            implicit none
    
            integer, intent(in) :: Nx, Ny, Nz, ist(3), ien(3), myid, ierr, var, nprocs
            integer, intent(in) :: wetting_param, num_ppt, ppt_rad(3)
            integer, intent(inout) :: bonus_start
            character, intent(in) :: start*10
            ...
            integer :: phi_count, phi_tot
            integer :: num_boundary_locations, num_boundary_locations_glob
            integer :: boundary_start(3), boundary_start_glob(3, nprocs)
            integer :: ix2, jy2, kz2, rad_ppt, root_id
            ...
            ! ===============================
            ! MPI Scan to find relative location in array
            ...
            boundary_start = (/ myid, boundary_start_index, boundary_start_index + num_boundary_locations /)
  
            ! Append all boundary_start data to a global array
            call MPI_Allgather(boundary_start, 3, MPI_INTEGER, &
                boundary_start_glob, 3, MPI_INTEGER, MPI_COMM_WORLD, ierr)

            ! Make boundary_location list on each process
            if (num_boundary_locations_glob > num_ppt) then
                  ...
                  do j  = ist(2), ien(2)
                    do i  = ist(1), ien(1)
                      if (boundary(i,j,k) == wetting_param) then
                        ...
                        print *, "These are the boundaries: ", i, j, k
                      end if
                    end do
                  end do
                end do
              ...
              do while (nppt.le.num_ppt)
                ...
                if (myid.eq.0) &
                  ...
                  ! TODO iseed is different here
  
                call MPI_Bcast(ll, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
                print *, myid, " myid, ll= ", ll
  
                ! Loop through boundary_start_glob to find root_id
                do i = 1, nprocs
                    if (boundary_start_glob(2, i) <= ll .and. ll <= boundary_start_glob(3, i)) then
                        root_id = boundary_start_glob(1, i)
                        exit ! Exit loop once root_id is found
                    end if
                end do
  
                ! If the random number falls to this process
                if (myid.eq.root_id) then
                  print *, myid, boundary_locations
                  ix = boundary_locations(ll, 1)
                  jy = boundary_locations(ll, 2)
                  kz = boundary_locations(ll, 3)
                  ! Check if ijk is ist ien
                  ! check if it is in the list of boundary locations
                  print *, "this is the chosen location: ", ix, jy, kz, "for ll=", ll
                end if
  
                call MPI_Bcast(ix, 1, MPI_INTEGER, root_id, MPI_COMM_WORLD, ierr)
                call MPI_Bcast(jy, 1, MPI_INTEGER, root_id, MPI_COMM_WORLD, ierr)
                call MPI_Bcast(kz, 1, MPI_INTEGER, root_id, MPI_COMM_WORLD, ierr)
                ...
                if (nppt>1) then
                  do ll = 1, nppt-1
                    ...
                    if (myid.eq.0) then 
                      write(*, '(A, 10I8)') "nppt, num_ppt, ll, ijk1, ijk2, num_boundary_locations_glob: ", nppt, num_ppt, ll, ix, jy, kz, ix2, jy2, kz2, num_boundary_locations_glob
                      write(*, '(A, 2I8)') "radius, rad_ppt: ", radius, rad_ppt
                    end if
                  end do
                end if
                ...
                print *, "myid, phi_count, phi_tot: ", myid, phi_count, phi_tot
      
                if (phi_tot .gt. 0) then
                  if (myid.eq.0) print *, phi_tot, " overlapping for ll ", ll
                else !(phi_tot .eq. 0) then  
                  ...
                  if (myid.eq.0) print *, nppt, " nppt worked!"
      
                  ! Add precipitate location to list for overlap checking
                  ll = nppt
                  precipitate_locations(ll, 1) = ix
                  precipitate_locations(ll, 2) = jy
                  precipitate_locations(ll, 3) = kz
                  print *, "assigning precipitate to : ", ix, jy, kz
...
!              call assign_concentrations(Nx, Ny, Nz, con, ist, ien, var, ierr, start, con_0_ppt)

            else
              write(*,'(A, I8)') "not enough boundary locations: ", num_boundary_locations_glob 

              ! Try again after double the number of initial iterations
              bonus_start = 2 * bonus_start
      ...
      subroutine assign_concentrations(Nx, Ny, Nz, con, ist, ien, var, ierr, start, con_0_ppt)
            implicit none
    
            integer, intent(in) :: Nx, Ny, Nz, ist(3), ien(3), ierr, var
            character, intent(in) :: start*10
            real *8, intent(in) :: con_0_ppt 
            real *8, intent(inout) :: con(ist(1):ien(1), ist(2):ien(2), ist(3):ien(3))
    
            integer :: i, j, k, ii, jj, kk, ivar
    
            if(myid.eq.0) print *, "assigning additional concentrations"

            ! Assign concentrations using start information 
            if (start.eq.'polycryst' .or. start.eq.'poly_bonus') then
            ...
            ! TODO assign additional concentrations for circle config
        
      end subroutine assign_concentrations
